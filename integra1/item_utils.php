...
class ItemUtils
{
	...
	public static function GetSSFItem($sku)
	{
		...
		$item['image'] = "http://catalog.eocenterprise.com/img/${mpn}";
        ...
        file_put_contents('/tmp/sx0', "$brandId - $dotIdx - $mpn - " . print_r($item, true), FILE_APPEND);
		...
		if (!empty($brandId))
        {
            $q = query(
                ...
                $mpn, $brandId);
            //file_put_contents('/tmp/sx0', $q, FILE_APPEND);
            $res = mysql_query($q);
        }
		else
	...
	public static function GetIMCItem($sku)
	{
		...
		foreach ($tryMPN as $mpn)
		{
			...
			if (!empty($row))
			{
				...
				$item['weight'] = $row[2];
				$item['price'] = $row[3];
				$item['image'] = "http://catalog.eocenterprise.com/img/${mpn}.jpg";
...
EOQ;
    ...
    public static function FindOrderItems($orderId, $keyword)
    {
        ...
        $q = <<<EOQ
			...
			AND s.status != 4
			AND TRIM(LEADING '0' FROM dss.order_id) = TRIM(LEADING '0' FROM '%s')
			AND (LCASE(si.description) LIKE '%%%s%%' OR LCASE(si.sku) LIKE '%%%s%%')
EOQ;
        $res = mysql_query(query($q, $orderId, $keyword, $keyword));
        ...
        while ($row = mysql_fetch_row($res))
        {
            ...
            $ret[] = ['id' => $row[0], 'sku' => $sku, 'desc' => $row[2]];
        }
