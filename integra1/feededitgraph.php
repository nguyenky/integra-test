...
$q=<<<EOQ
	SELECT DATE_FORMAT( edited_on,  '%m/%d %H:00' ) AS HR, edited_by, COUNT(DISTINCT gf.mpn) AS CNT
	FROM integra_prod.google_feed_log_edit gfl, integra_prod.google_feed gf
	WHERE edited_on >= DATE_SUB( CURDATE( ) , INTERVAL ${graphDays} DAY )
...
$q=<<<EOQ
	SELECT DATE_FORMAT( created_on,  '%m/%d %H:00' ) AS HR, created_by, COUNT(DISTINCT mpn) AS CNT
...
<html>
<head>
	<title>Google Feed Editing Productivity Graph</title>
	<link rel="stylesheet" type="text/css" href="css/stats.css">
</head>
<body>
	<br/>
	<center>
        <h1>Google Feed Editing Productivity Graph</h1>
        <br>
        <form>
            Days in graph:
            <select name="gd">
