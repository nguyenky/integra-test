...
// DisplayLoadingImage              CombineUrl                  dgGetDbDriverType 
// IsOperationCompleted             SetSqlLimit                 dgGetAffectedRows 
// IgnoreBaseTag                    SetSqlLimitByDbType         dgFetchRow    
// ExecuteSQL                       SetRealEscapeStringByDbType dgQuery
...
class DataGrid
{
    ...
    // Data Members
    //==========================================================================
    static private $dgVersion = '7.9.9';
    ...
    function __construct($debug_mode = false, $messaging = true, $unique_prefix = ''){
        ...
        if(preg_match("/'/", $php_self) || preg_match('/:/', $php_self)){
    ...
    protected function DrawControlPanel(){       
        ...
        if($this->filteringAllowed || $this->exportingAllowed || $this->printingAllowed || !empty($this->navigationBar)){
            ...
            if($this->filteringAllowed && $this->filteringHideIconAllowed && (($this->mode != 'edit') && ($this->mode != 'details'))){
                ...
                if(!$this->isPrinting){
                    echo '<a id="'.$this->uniquePrefix.'a_hide" style="cursor:pointer;'.$hide_display.'" onclick="return _dgHideUnHideFiltering(\'hide\', \''.$this->uniquePrefix.'\');"><img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/search_hide.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/search_hide.gif' : $this->directory.'images/search_hide.gif').'" alt="'.$this->lang['hide_search'].'" title="'.$this->lang['hide_search'].'" /></a>';
                    echo '<a id="'.$this->uniquePrefix.'a_unhide" style="cursor:pointer;'.$unhide_display.'" onclick="return _dgHideUnHideFiltering(\'unhide\', \''.$this->uniquePrefix.'\');"><img aclass="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/search_unhide.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/search_unhide.gif' : $this->directory.'images/search_unhide.gif').'" alt="'.$this->lang['unhide_search'].'" title="'.$this->lang['unhide_search'].'" /></a>';
                }
                echo '</td>'.$this->nl;
            }
            if(!$this->hideGridBeforeSearch || ($this->hideGridBeforeSearch && $this->onSubmitFilter != '')){
                if($this->exportingAllowed && $req_mode != 'add'){
                    if((($req_export == '') || !$this->isPrinting) && ($this->isPrinting == '')){
                        if($this->arrExportingTypes['csv'] == true){
                            ...
                            echo '<img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/csv.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/csv.gif' : $this->directory.'images/csv.gif').'" alt="'.$this->lang['export_to_excel'].'" title="'.$this->lang['export_to_excel'].' (csv)" />';
                            echo '</a></td>'.$this->nl;
                        }
                        if($this->arrExportingTypes['xls'] == true){
                            ...
                            echo '<img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/xls.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/xls.gif' : $this->directory.'images/xls.gif').'" alt="'.$this->lang['export_to_excel'].'" title="'.$this->lang['export_to_excel'].' (xls)" />';
                            echo '</a></td>'.$this->nl;                          
                        }
                        if($this->arrExportingTypes['pdf'] == true){
                            ...
                            echo '<img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/pdf.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/pdf.gif' : $this->directory.'images/pdf.gif').'" alt="'.$this->lang['export_to_pdf'].'" title="'.$this->lang['export_to_pdf'].'" />';
                            echo '</a></td>'.$this->nl;
                        }
                        if($this->arrExportingTypes['xml'] == true){
                            ...
                            echo '<img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/xml.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/xml.gif' : $this->directory.'images/xml.gif').'" alt="'.$this->lang['export_to_xml'].'" title="'.$this->lang['export_to_xml'].'" />';
                            echo '</a></td>'.$this->nl;                                                      
                        }
                        if($this->arrExportingTypes['doc'] == true){
                            ...
                            echo '<img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/doc.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/doc.gif' : $this->directory.'images/doc.gif').'" alt="'.$this->lang['export_to_word'].'" title="'.$this->lang['export_to_word'].'" />';
                            echo '</a></td>'.$this->nl;
                        }
                    }else{
                ...
                if($this->printingAllowed && $req_mode != 'add' && $this->rowsTotal > 0){
                    if(($req_export == '') && !$this->isPrinting){
                        ...
                        echo '<td class="dg_exi_td">';
                        if($this->printType == 'new') echo '<a href="javascript:_dgPopupPrint(\''.$this->uniquePrefix.'\',\''.$this->cssClass.'\')" class="'.$this->cssClass.'_dg_a"><img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/print.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/print.gif' : $this->directory.'images/print.gif').'" alt="'.$this->lang['printable_view'].'" title="'.$this->lang['printable_view'].'" /></a>';
                        else echo '<a style="cursor:pointer;" onclick="javascript:'.$this->uniquePrefix.'_doPostBack(\'print\',\'\',\''.$print_curr_url.'\');" class="'.$this->cssClass.'_dg_a"><img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/print.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/print.gif' : $this->directory.'images/print.gif').'" alt="'.$this->lang['printable_view'].'" title="'.$this->lang['printable_view'].'" /></a>';
                        echo '</td>'.$this->nl;
                    }else{
                        echo '<td align="right"><a style="cursor:pointer;" onclick="window:print();" class="'.$this->cssClass.'_dg_a no_print" title="'.$this->lang['print_now_title'].'">'.$this->lang['print_now'].'</a></td>'.$this->nl;
                    }
                }
            }
            if($this->filteringAllowed && $this->refreshPageIconAllowed && ($this->mode == 'view') && ($req_mode != 'update') && ($req_mode != 'delete') && ($req_mode != 'clone')){
                if(!$this->isPrinting){
                    ...
                    echo '<td class="dg_exi_td"><a style="cursor:pointer" onclick="'.str_replace('&', '&amp;', $href_string).'" class="'.$this->cssClass.'_dg_a"><img class="dg_opacity" src="'.(file_exists($this->directory.'styles/'.$this->cssClass.'/images/refresh.gif') ? $this->directory.'styles/'.$this->cssClass.'/images/refresh.gif' : $this->directory.'images/refresh.gif').'" alt="'.$this->lang['refresh_page'].'" title="'.$this->lang['refresh_page'].'" /></a></td>';
                }
            }        
            echo '</tr>';
            echo '</table>';
        ...
        if($this->layouts[$this->layoutType] == '0'){
            ...
            for($r = $this->rowLower; (($r >=0 && $this->rowUpper >=0) && ($r < $this->rowUpper) && ($r < $page_size)); $r++){
                ...
                for($c_sorted = $this->colLower; $c_sorted < $this->sortedColumnsSize; $c_sorted++){
                    ...
                    $field_name = $this->GetFieldName($c);
                    if($this->CanViewField($field_name)){
                        #// [#0069 under check - 22.07.13] 
                        $content .= '<td>'.str_replace(',', '', strip_tags($this->GetFieldValueByType($row[$c], $c, $row))).'</td>';
                    }                
                }
                $content .= '</tr>';
            }        
            ...
            for($r = $this->rowLower; (($r >=0 && $this->rowUpper >=0) && ($r < $this->rowUpper) && ($r < $page_size)); $r++){
                ...
                for($c_sorted = $this->colLower; $c_sorted < $this->sortedColumnsSize; $c_sorted++){
                    ...
                    if($this->CanViewField($field_name)){
                        ...
                        #// $value = ($fp_tooltip) ? $row[$c] : str_replace(',', '', strip_tags($this->GetFieldValueByType($row_c, $c, $row)));
                        #// old - $value = str_replace(',', '', strip_tags($row[$c]));
                        $value = str_replace(',', '', strip_tags($this->GetFieldValueByType($row_c, $c, $row)));
                        if($export_type == 'xls'){
                            $size = strlen($value);
                ...
                for($c_sorted = $this->colLower; $c_sorted < $this->sortedColumnsSize; $c_sorted++){
                    ...
                    if($this->CanViewField($field_name)){
                        if($this->IsForeignKey($field_name)){
                            ...
                            #// [#0069 under check - 22.07.13] 
                            $data_content .= str_replace("\t", '', strip_tags($this->GetFieldValueByType($row_c, $c, $row)));
                        }                        
                    ...
                    switch($ff_type){
                        ...
                        case 'calendar':
                            ...
                            if($fp_calendar_type == 'floating'){
                                ...
                                $output .= '<img id="img_'.$textbox_id.'" src="'.$this->include_path.'styles/'.$this->cssClass.'/images/cal.gif" alt="'.$this->lang['set_date'].'" title="'.$this->lang['set_date'].'" align="top" style="cursor:pointer;margin:3px;margin-left:6px;margin-right:6px;border:0px;" ';
                                ...
                                $output .= '<a class="'.$this->cssClass.'_dg_a2" href="javascript:_dgOpenCalendar(\''.(($this->ignoreBaseTag) ? $this->HTTP_HOST.'/' : '').$this->directory.'\',\'separator='.$this->dtSeparator.'\',\'frmFiltering'.$this->uniquePrefix.'\',\'\',\''.$this->uniquePrefix.'_ff_'.$table_field_name.'_'.$fldValue_field.$fp_field_type.'\',\''.$date_format.'\')"><img src="'.$this->include_path.'styles/'.$this->cssClass.'/images/cal.gif" alt="'.$this->lang['set_date'].'" title="'.$this->lang['set_date'].'" align="top" style="border:0px;margin:3px 6px;" /></a>'.$this->nbsp;
                            }
                            break;
                        default:
    ...
    protected function DrawCustomized(){
        ...
        if($this->isLoadingImageEnabled && $this->methodPostBack != 'ajax') echo '<div id="'.$this->uniqueRandomPrefix.'loading_image" class="dg_loading_image">'.$this->lang['loading_data'].' <img src="'.$this->include_path.'images/loading.gif" alt="'.$this->lang['loading_data'].'" /></div>'.$this->nl;
        ...
        if(isset($this->templates[$this->layoutType]['header'])){
            ...
            if($this->sortingAllowed){
                ...
                for($c_sorted = $this->colLower; $c_sorted < $this->sortedColumnsSize; $c_sorted++){
                    ...
                    $header_link = '<a class="'.$this->cssClass.'_dg_a_header" href="javascript:void(\'sort\');" onclick="'.$href_string.'" title="'.$field_titile_tooltip.'"><b>'.$this->GetHeaderName($c_field_name).'</b>';
        ...
        if($req_mode == 'add' || $req_mode == 'edit' || ($req_mode == 'update' && $this->rid == '-1')){
            ...
            echo '<form name="'.$this->uniquePrefix.'frmEditRow" id="'.$this->uniquePrefix.'frmEditRow" method="post" action="'.(($this->methodPostBack == 'get') ? $curr_url : '').'">'.$this->nl;
    ...
    protected function DrawTabular(){
        ...
        if($this->isLoadingImageEnabled && $this->methodPostBack != 'ajax') echo '
        <div id="'.$this->uniqueRandomPrefix.'loading_image" class="dg_loading_image">'.$this->lang['loading_data'].' <img src="'.$this->include_path.'images/loading.gif" alt="'.$this->lang['loading_data'].'" /></div>'.$this->nl;
                        ...
                        if($this->CanViewField($field_name)){ 
                            ...
                            if($this->sortingAllowed && $fp_sortable){
                                ...
                                if(!$this->isPrinting){
                                    ...
                                    if($req_sort_field && ($c == ($req_sort_field -1))){
                                        echo $this->nbsp.'<span title="'.$sort_title.'" class="dg_sort_arrow">'.$sort_sign.'</span>';
                                    }
                                    echo '</a>';
                                }else{
                                    echo '<b>'.$this->GetHeaderName($field_name).'</b>';                            
    ...
    protected function DrawColumnar(){
        ...
        if($this->isLoadingImageEnabled && $this->methodPostBack != 'ajax') echo '<div id="'.$this->uniqueRandomPrefix.'loading_image" class="dg_loading_image">'.$this->lang['loading_data'].' <img src="'.$this->include_path.'images/loading.gif" alt="'.$this->lang['loading_data'].'" /></div>'.$this->nl;
        echo '<form style="margin:10px;padding:0px;" name="'.$this->uniquePrefix.'frmEditRow" id="'.$this->uniquePrefix.'frmEditRow" method="post" action="'.(($this->methodPostBack == 'get') ? $curr_url : '').'">'.$this->nl;
        echo '<input type="hidden" name="'.$this->uniquePrefix.'_operation_randomize_code" value="'.$this->GetRandomString(20).'" />'.$this->nl;
        $this->PrintHiddenFieldsFromUrl($curr_url);
            ...
            echo $this->nl.'<table border="0">
                  <tr>
                    <td align="'.(($this->direction == 'ltr') ? 'left' : 'right').'" class="dg_nowrap">';
                        ...
                        echo '<img tyle="margin:0px;padding:0px;border:0px;" src="'.$this->directory.'styles/'.$this->cssClass.'/images/arrow_ltr.png" width="38" height="22" alt="" />';
                        echo '</td><td>';
                        if(!$this->isPrinting){
                        ...
                        if($count > 0){
                            ...
                            foreach($this->arrMultirowOperations as $key => $val){
                                if($this->arrMultirowOperations[$key]['view']){
                                    ...
                                    if($this->multirowViewType == 'dropdownlist'){
                                        echo '<option value="'.$key.'"'.(!empty($flag_name) ? ' flag_name="'.$flag_name.'" flag_value="'.$flag_value.'"' : '').' title="'.$tooltip.'">'.$key.'</option>';
                                    }else{
    ...
    protected function PagingSecondPart($lu_paging=false, $upper_br, $lower_br, $type='1'){
        ...
        if($this->pagesTotal >= 1){
            ...
            if($lu_paging['results'] || $lu_paging['pages'] || $lu_paging['page_size']){
                if($upper_br) $text .= '';  
                $text .= $this->nl.'<form name="frmPaging'.$this->uniquePrefix.$type.'" id="frmPaging'.$this->uniquePrefix.$type.'" action="" style="margin:0px;padding:0px;">';
            ...
            foreach($_POST as $fldName => $fldValue){
                ...
                if(!$this->IsExceptedField($fldName, 'add') && !$this->IsValidator($fldName)){
                    ...
                    if($this->IsDate($fldName)){
                        ...
                        if(trim($fldValue) != ''){
                            if($this->convertOutput) $fldValue = hebrev(mb_convert_encoding($fldValue, $this->convertFrom, $this->convertTo));
                            if($this->dgGetDbDriverType() == 'oci8') $sql_values .= $fldValue;
                            else $sql_values .= '\''.$fldValue.'\'';
                        }else if($this->IsFieldRequired($fldName)){
                            $sql_values .= '0';    
                        }else{
                            $sql_values .= 'NULL';    
                ...
                switch($field_type){
                    ...
                    case 'image':
                        ...
                        if($fp_default != ''){
                            if($this->IsFileExists($fp_default)){
                                $img_default = '<img src="'.$fp_default.'" '.$fp_image_width_attr.$fp_image_height_attr.' alt="" '.$on_js_event.'>';
                            }else{
                    ...
                    case 'label':
                        ...
                        if(($field_value_trim != '')
                            ...
                            $output = $this->nbsp.'<label class="'.$this->cssClass.'_dg_label" id="'.$element_id.'" '.$title.' '.$on_js_event.'>'.trim($field_value).'</label>'.$this->nbsp;
                ...
                if($mode === 'edit'){
                    ...
                    switch($field_type){
                            ...
                            switch($fp_view_type){
                                case 'radiobutton':
                                    if(is_array($this->columnsEditMode[$field_name]['source'])){  // don't remove columns_edit_mode
                                        $ret_enum .= $this->nbsp.$this->DrawRadioButtons($full_field_name, $field_name, $this->columnsEditMode[$field_name]['source'], $field_value, 'source', '', $disabled, $on_js_event, $fp_elements_alignment).$this->nbsp;
                                    }else{
                                        $ret_enum .= $this->nbsp.$this->DrawRadioButtons($full_field_name, $field_name, $this->GetEnumValues($this->tblName, $field_name), $field_value, 'source', '', $disabled, $on_js_event, $fp_elements_alignment).$this->nbsp;
                                    }                                        
                                    break;                            
                                case 'checkbox':
                                    ...
                                    if(is_array($this->columnsEditMode[$field_name]['source'])){  // don't remove columns_edit_mode                                    
                                        $ret_enum .= $this->nbsp.$this->DrawCheckBoxes($full_field_name, '', $this->columnsEditMode[$field_name]['source'], $field_value, '', '', '', $disabled, $on_js_event, $enum_multiple, $fp_multiple_size, $fp_elements_alignment).$this->nbsp;
                                    }else{
                                        $ret_enum .= $this->nbsp.$this->DrawCheckBoxes($full_field_name, '', $this->GetEnumValues($this->tblName, $field_name), trim($field_value), '', '', '', $disabled, $on_js_event, $enum_multiple, $fp_multiple_size, $fp_elements_alignment).$this->nbsp;
                                    }
                                    break;    
                                case 'dropdownlist':
                                default:
                        ...
                        case 'file':
                            ...
                            if($this->GetFieldProperty($field_name, 'host') == 'remote'){                                
                                ...
                                if($file == true){
                                    ...
                                    $ret_file .= '<td><input type="text" class="'.$this->cssClass.'_dg_textbox" '.$field_width.' title="'.$this->GetFieldTitle($field_name).'" name="'.$full_field_name.'" id="'.$full_field_name.'" '.$disabled.' '.$on_js_event.' /></td><td nowrap="nowrap" width="7px"></td>';
                                                ...
                                                if(isset($_FILES[$full_field_name]['tmp_name'])){
                                                    if($field_type == 'file' && $fp_save_as == 'blob'){
                                                        ...
                                                        if($req_mode != 'add'){
                                                            if($this->dbType == 'PEAR'){
                                                                $dSet = $this->dbHandler->query($sql);
                                                                $affected_rows = $this->dgGetAffectedRows();
                                                            }else{
                                                                try{
                        ...
                        case 'textarea':
                            ...
                            $texarea  = $this->nbsp.'<textarea class="'.$this->cssClass.'_dg_textarea'.$field_class.'" id="'.$full_field_name.'" name="'.$full_field_name.'" title="'.$this->GetFieldTitle($field_name).'" rows="'.$field_rows.'" cols="'.$field_cols.'" '.$field_maxlength.' '.$field_width.' '.$readonly.' '.$on_js_event.$placeholder.'>'.trim($field_value).'</textarea>'.$this->nbsp;
                        ...
                        case 'textbox':                           
                            ...
                            $output = '<input class="'.$this->cssClass.'_dg_textbox" '.$field_width.' type="text" title="'.$this->GetFieldTitle($field_name).'" name="'.$full_field_name.'" id="'.$full_field_name.'" value="'.trim($field_value).'" '.$field_maxlength.' '.$readonly.' '.$on_js_event.' '.$autocomplete.' '.$template.$placeholder.' />';
                            break;
                        case 'time':
                    ...
                    switch($field_type){
                        ...
                        case 'image':
                            ...
                            $fp_image_height_attr = (trim($fp_image_height) != '') ? ' height="'.$fp_image_height.'"' : '';
                            $fp_default       = $this->GetFieldProperty($field_name, 'default', 'edit', 'normal');
                            $img_default      = (($fp_default != '') && $this->IsFileExists($fp_target_path.trim($fp_default))) ? '<img src="'.$fp_target_path.$fp_default.'" '.$fp_image_width_attr.$fp_image_height_attr.' alt="" />' : '<span class="'.$this->cssClass.'_dg_label">'.$this->lang['no_image'].'</span>';
                            ...
                            if((trim($field_value) !== '') && $this->IsFileExists($fp_target_path.trim($field_value))){
                                ...
                                $ret_image = $ret_image_img;
                                if($fp_allow_downloading && !$this->isPrinting) $ret_image .= '<br />[<img src="'.$this->directory.'images/download.png" alt="" style="margin-bottom:-3px" border="0" /> <a class="'.$this->cssClass.'_dg_a" href="'.$img_src.'" title="'.$this->lang['click_to_download'].'">'.$this->lang['download'].'</a>]';
                                $output = $ret_image;                            
                            }else{
    ...
    protected function PrepareTooltip($field_name = '', $mode = 'view'){
        ...
        if(!$this->isPrinting && $fp_header_tooltip != ''){
            ...
            $output = ' <img src="'.$this->directory.'styles/'.$this->cssClass.'/images/question_mark.gif" class="'.$this->cssClass.'_dg_question_mark" alt="" '.$tooltip_title.' />';
        }
        return $output;        
    }
...
';
    ...
    protected function DrawCalendarButton($field_name, $field_type, $datetime_format='Y-m-d', $field_value='', $field_width='', $field_maxlength='', $on_js_event='', $readonly=false, $fp_calendar_type = 'popup', $multirow_postfix = ''){
        ...
        if($fp_calendar_type == 'floating'){
            ...
            if(!$readonly) $ret_date .= "<img id='img".$full_field_name."' src='".$this->include_path.'styles/'.$this->cssClass."/images/cal.gif' alt='".$this->lang['set_date']."' title='".$this->lang['set_date']."' align='top' style='cursor:pointer;margin:3px;margin-left:6px;margin-right:6px;border:0px;' />".$this->nbsp.$this->ScriptOpen()."Calendar.setup({firstDay : ".$this->weekStartingDay.", inputField : '".$full_field_name."', ifFormat : '".$if_format."', showsTime : ".$show_time.", button : 'img".$full_field_name."'});".$this->ScriptClose();
            ...
            $ret_date  = $this->nbsp."<input class='".$this->cssClass."_dg_textbox' ".$field_width." readonly type='text' title='".$fp_title."' name='".$full_field_name."' id='".$full_field_name."' value='".$this->MyDate($field_value, $field_type, $show_seconds)."' $field_maxlength ".$on_js_event.' />';
            if(!$readonly) $ret_date .= '<a class="'.$this->cssClass.'_dg_a2" title="'.$fp_title.'" href="javascript:_dgOpenCalendar(\''.(($this->ignoreBaseTag) ? $this->HTTP_HOST.'/' : '').$this->directory.'\',\'separator='.$this->dtSeparator.'\',\''.$this->uniquePrefix.'frmEditRow\',\''.$full_field_name.'\',\''.$field_name.'\',\''.$field_type.'\')"><img src="'.$this->include_path.'styles/'.$this->cssClass.'/images/cal.gif" alt="'.$this->lang['set_date'].'" title="'.$this->lang['set_date'].'" align="top" style="margin:3px;margin-left:6px;margin-right:6px;border:0px;" /></a>'.$this->nbsp;
        }
    ...
    private function DrawColorPicker($full_field_name, $field_value, $field_name, $disabled, $on_js_event=''){
        $output  = '<img src="'.$this->directory.'images/colors.png" alt="" align="top" /> ';
        ...
        if(!$this->isError){
            if($this->dbType == 'PEAR'){
                if(!$this->dbHandler->isError($this->dataSet)){
                    $fields = $this->dataSet->tableInfo();
                    if(isset($fields[$field_offset]['name'])) return $this->FieldNameByCase($fields[$field_offset]['name']);
                }
            }else{
                if($field_offset != '-1'){
            ...
            if(!$this->isPrinting){
                switch($mode_type){
                    ...
                    case 'image':                        
                        ...
                        $return_value .= $nbsp."<img class='dg_opacity dg_pointer' onclick=".$on_click." src='".$this->directory.'styles/'.$this->cssClass."/images/".$image_file."' alt='$alt_name' title='$alt_name' />".$nbsp;
    ...
    private function GetExportHeaderTooltip($field_name, $mode, $doc_type = ''){
        $fp_header_tooltip = $this->GetFieldProperty($field_name, 'header_tooltip', $mode);
        $output = ($doc_type == 'xml') ? ' title="'.$fp_header_tooltip.'"' : ' ('.$fp_header_tooltip.')';
