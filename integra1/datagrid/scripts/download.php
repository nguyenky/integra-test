...
################################################################################
    ...
    if(preg_match('/0/', $file) || preg_match('/0/', $dir)){
        echo 'preg_match. Cannot find export file! Turn on debug mode to see more info.';
    }
    
    if(in_array($file, array('export.xml', 'export.csv', 'export.xls', 'export.pdf', 'export.doc')) && @file_exists($file_path) && is_file($file_path)){
        ...
        if($debug){
            ...
            echo 'Cannot find export file! Turn on debug mode to get more info. NOt support file';                                         
        }        
    }
    exit(0);
