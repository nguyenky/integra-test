...
function Output($name='', $dest='', $debug='')
{
	...
	switch($dest)
	{
		...
		case 'F':
			// Save to local file
			// [#001] added by ApPHP 27.07.2009
			if($debug == true) $f = fopen($name,'wb');
			else $f = @fopen($name,'wb');
			if(!$f){
				if($debug == true) $this->Error('Unable to create output file: '.$name);
				else $this->Error("<span style='color:#a61100'>Unable to create output file. <br />For more information turn on debug mode.</span>");
			}
