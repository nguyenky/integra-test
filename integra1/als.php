...
FROM
(SELECT DATE_FORMAT(edited_on, '%Y-%m') AS month_created, aml.asin, IFNULL((SELECT 1 FROM eoc.sales_items si WHERE si.amazon_asin = aml.asin LIMIT 1), 0) AS sold
FROM integra_prod.amazon_monitor_log aml
WHERE aml.is_new = 1
AND edited_by = '{$user}'
AND edited_on >= '2017-05-01') x
GROUP BY 1
EOQ;
...
<html>
<head>
	<title>Amazon Listings Created Stats</title>
<link rel="stylesheet" type="text/css" href="css/stats.css">
</head>
<body>
	<br/>
	<center>
        <h1>Amazon New Listing Stats for <?=$firstName?> <?=$lastName?></h1>
