	...
	if(isset($_FILES['file'])) {
		if ($_FILES['file']['error'] == UPLOAD_ERR_OK && is_uploaded_file($_FILES['file']['tmp_name']))
		{
			if (move_uploaded_file($_FILES['file']['tmp_name'], $file))
				exec('nohup php ' . __DIR__ . DIRECTORY_SEPARATOR . 'jobs' . DIRECTORY_SEPARATOR . 'e_research_file.php ' . $userId . ' '. 2 .' >/dev/null 2>/dev/null &');
		}
	} else if(!empty($query))
	{
		$ch = curl_init();
...
?>
		<form id="searchFormV2" method="GET">
...
function selectFile()
{
	$('#searchFormV2').attr('method', $('#file').val() ? 'POST' : 'GET');
	$('#searchFormV2').attr('enctype', 'multipart/form-data');
}
