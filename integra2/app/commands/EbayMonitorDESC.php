...
class EbayMonitorDESC extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'ebay:monitor-desc';
	...
	public function fire()
	{
		...
		try {
			...
			$rows = EbayMonitorNew::select('item_id','vs_ours','strategy')->where('last_scraped','<',\Carbon\Carbon::now()->subDays(1))->where('disable',0)->where('strategy','<>',99)->orderBy('last_scraped','DESC')->limit(150)->get();
			$ids=[];
			$dbItems = [];
			$strategies = [];
			...
			foreach($subIds as $id) {
				...
				$res = file_get_contents("http://open.api.ebay.com/shopping?callname=GetMultipleItems&responseencoding=XML&appid=" . Config::get('integra.ebay.APP_ID2') . "&siteid=0&version=847&ItemID=${idsStr}&IncludeSelector=Details,ShippingCosts");
				EbayApiCallCounter::create([
							'ebay_service_name'	=> 'GetMultipleItems',
							'feature_name'		=> 'Ebay Monitor',
							'token'				=> Config::get('integra.ebay.APP_ID2')
						]); 
